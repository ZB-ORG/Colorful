...
//#define RISING_EDGE_TRIG	//上升沿中断
#define FALLING_EDGE_TRIG	//下降沿中断

extern void SIRQ_Int(void);
//使用GPIO_A22作为SIRQ
void SIRQ_Init(void)
{
	//A22设置为SIRQ0
	act_writel( act_readl(MFP_CTL0) &0xf8ffffff |0x05000000, MFP_CTL0);
    

#if defined(RISING_EDGE_TRIG)
	act_writel(act_readl(INTC_EXTCTL) & (~2), INTC_EXTCTL);

	//设置下拉
	act_writel(act_readl(PADPUPD) | (1 << PADPUPD_SIRQ0PD), PADPUPD);
	act_writel(act_readl(PADPUPD) & (~(1 << PADPUPD_SIRQ0PU)), PADPUPD);
#elif defined(FALLING_EDGE_TRIG)
	act_writel(act_readl(INTC_EXTCTL) | 0x02, INTC_EXTCTL);
    ...
    //disable SIRQ0 and SIRQ1 interrupt pending mask
    act_writel(act_readl(INTC_MSK) & (~(1<<INTC_MSK_SIRQ0_IM) ), INTC_MSK);
    
    // set SIRQ0 and SIRQ1 interrupt source
    ...
    act_writel( act_readl(INTC_EXTIP) | 1, INTC_EXTIP);

	sys_request_irq(IRQ_SIRQ0, SIRQ_Int);


/*
	//SIRQ0触发器使能
	//这一句不能打开，因为A3是模拟输入口
	//act_writel(act_readl(PAD_SMIT0) | (1 << PAD_SMIT0_P_GPIOA3), PAD_SMIT0);

#if defined(RISING_EDGE_TRIG)
	act_writel(act_readl(INTC_EXTCTL) & (~2), INTC_EXTCTL);

	//设置下拉
	act_writel(act_readl(PAD_PUPD) | (1 << PAD_PUPD_SIRQ0_PD), PAD_PUPD);
	act_writel(act_readl(PAD_PUPD) & (~(1 << PAD_PUPD_SIRQ0_PU)), PAD_PUPD);
#elif defined(FALLING_EDGE_TRIG)
	act_writel(act_readl(INTC_EXTCTL) | 0x02, INTC_EXTCTL);
	...
	//清除中断标志
	act_writel(act_readl(INTC_EXTIP) | 1, INTC_EXTIP);
	//不用使能MASK,因为下面的函数中会使能
	//act_writel(act_readl(INTC_MSK) | (1 << IRQ_SIRQ0), INTC_MSK);
	sys_request_irq(IRQ_SIRQ0, SIRQ_Int);
*/
}
