...
void __section__(".text.autoplay") tws_vol_sync(void)
{
    ...
    vol = (uint16) ((sys_comval->volume_current * 0x7f) / VOLUME_VALUE_MAX);
    msg_apps_t msg;
...
app_result_e get_message_loop(void)
{
    ...
    while (1)
    {
       ...
       /*
       if( (g_btplay_cur_info.status != g_last_btplay_status)&&(g_bt_stack_cur_info.rmt_dev[g_bt_stack_cur_info.a2dp_active_id].a2dp_status==A2DP_STATUS_PLAY)
        &&(g_btplay_cur_info.status==A2DP_STATUS_PLAY))
       {
           g_btplay_check_btplay_status_timer = cur_ab_timer;
           need_check_status = TRUE;
           PRINT_INFO("A1");
       }
       else if(g_bt_stack_cur_info.rmt_dev[g_bt_stack_cur_info.a2dp_active_id].a2dp_status==A2DP_STATUS_PAUSE)
       {
         //   PRINT_INFO("A2");
            g_last_btplay_status_before_unlink=A2DP_STATUS_PAUSE;
       }      
       else if(g_bt_stack_cur_info.rmt_dev[g_bt_stack_cur_info.a2dp_active_id].a2dp_status!=A2DP_STATUS_PLAY)
       {
            g_btplay_check_btplay_status_timer = 0;
