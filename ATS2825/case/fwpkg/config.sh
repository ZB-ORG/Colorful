...
#读取板型配置
BoardType=`grep "BoardType" $int_file | awk -F '=' '{print $2}'`

#读取config.txt配置
config_txt_name=`grep "config_txt" $int_file | awk -F '=' '{print $2}'`

#读取fwimage.cfg配置
config_cfg_name=`grep "fwimage_cfg" $int_file | awk -F '=' '{print $2}'`

#读取是否支持TWS
SupportTWS=`grep "SupportTWS" $int_file | awk -F '=' '{print $2}'`

#读取是否支持OTA
SupportOTA=`grep "SupportOTA" $int_file | awk -F '=' '{print $2}'`

#读取是否支持Waves音效
SupportWaves=`grep "SupportWaves" $int_file | awk -F '=' '{print $2}'`

#读取是否支持SPP/BLE
SupportSPPBLE=`grep "SupportSPPBLE" $int_file | awk -F '=' '{print $2}'`

#读取是否支持ESD
SupportESD=`grep "SupportESD" $int_file | awk -F '=' '{print $2}'`

#读取是否支持固定采样率
SupportFIXSAMPLE=`grep "SupportFIXSAMPLE" $int_file | awk -F '=' '{print $2}'`

#读取dae_config.txt配置
cfg_dae_txt_name=`grep "cfg_dae_txt" $int_file | awk -F '=' '{print $2}'`

SDKRelease=`grep "SDKRelease" $int_file | awk -F '=' '{print $2}'`
...
grep -q "\/\/\#define ENABLE_TRUE_WIRELESS_STEREO" ../inc/btstack_common.h
if [ $? == 0 ]
then
...
grep -q '//FWIM="otatest.ap"' ./config_txt/$config_cfg_name
if [ $? == 0 ]
then 
